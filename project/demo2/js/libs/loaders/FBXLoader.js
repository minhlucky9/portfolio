 ...
 */
	...
	class FBXLoader extends THREE.Loader {
		...
		constructor(manager, isLoadPlayer) {
			super(manager);
			isPlayer = false;
			if(isLoadPlayer) {
				isPlayer = isLoadPlayer;
			}
		}
	...
	class FBXTreeParser {
			...
			if ('Deformer' in fbxTree.Objects) {
				...
				for (const nodeID in DeformerNodes) {
					...
					if (deformerNode.attrType === 'Skin') {
						...
						//
						var modelID = connections.get(parseInt(relationships.parents[0].ID)).parents[0].ID;
						var modelName = fbxTree.Objects.Model[modelID].attrName;

						const skeleton = this.parseSkeleton(relationships, DeformerNodes, modelName);
						skeleton.ID = nodeID;
		...
		parseSkeleton(relationships, deformerNodes, modelName) {

			const rawBones = [];
			if (isPlayer == false || modelName.toUpperCase().includes("Body_".toUpperCase()) || ValidatePartName(modelName)) {
				//
		...
		parseScene(deformers, geometryMap, materialMap) {
			...
			modelMap.forEach(function (model) {
				...
				//
				if(isPlayer == true && modelNode.attrType == "Mesh" && (!modelNode.attrName.toUpperCase().includes("Body_".toUpperCase()) && !ValidatePartName(modelNode.attrName))) {
					return;
				}
